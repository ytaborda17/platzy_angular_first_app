...
export class PlayerDialogComponent implements OnInit {
  ...
  private editPlayer(playerFormValue) {
    ...
    const playerFormWithFormattedKey = { ...playerFormValue, key: this.player.$key };
    delete playerFormWithFormattedKey.$key;
    const moddifiedPlayers = this.team.players ? 
    this.team.players.map(player => {
      return player.key === this.player.$key ? playerFormWithFormattedKey : player;
    }) : this.team.players;
    const formattedTeam = {
      ...this.team,
      players: [ ...(moddifiedPlayers ? moddifiedPlayers : [playerFormWithFormattedKey])]
    };
